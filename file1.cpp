...
for(auto temp = reservations.begin();temp!= reservations.end();++temp)
{if(temp->getreservation_id()==ID){
...
void SessionBase::set_createdAt(time_t c){
    createdAt=c;}
void SessionBase::set_lastlogin(time_t now){
    lastTimeLogin=now;}
...
namespace StudentSession{
    ...
    void SessionManager::login(string a,string b){}
...
namespace StudentSession {
    SessionManager::SessionManager() : currentStudent(nullptr), shopping_cart(nullptr), _studentID(0) {
    ...
    void SessionManager::save_session() {
        ...
        string fname = "student_" + to_string(currentStudent->getid()) + ".session";
        fs::path fp = dir / fname;
        ofstream fout(fp, ios::trunc);
        if (!fout.is_open()) return;
        fout << "student_id:" << currentStudent->getuserID() << "\n";
    ...
    void SessionManager::login(const string& identifier, const string& password) {
        ...
        while (getline(fin, line)) {
            ...
            bool id_match = false;
            if (!cols.empty() && cols[0] == identifier) id_match = true; 
            if (cols.size() >= 3 && cols[2] == identifier) id_match = true;
